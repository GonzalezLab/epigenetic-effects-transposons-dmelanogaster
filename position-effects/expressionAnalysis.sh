...
#do
mkdir -p $wDir/zscoreTMM/$assembly/$tissue/

> zscoreTMM/${assembly}/${tissue}//zscore_${tissue}-${assembly}.tab
while read gene
do
#TE=$(echo "$gene" | cut -f1)
...
awk -v gene="$gene" ' $1 == gene ' ../expression_analysis/RNA-seq/${assembly}/TMM_matrix_def.tab | awk -v tissue="$tissue" ' $5 == tissue ' | awk '{print $0 "\t" "TE"}' >> tmp/${assembly}/$tissue//$gene.TMM.matrix
...
awk -v gene="$gene" ' $1 == gene ' ../expression_analysis/RNA-seq/${strainNoTE}/TMM_matrix_def.tab | awk -v tissue="$tissue" ' $5 == tissue ' | awk '{print $0 "\t" "noTE"}' >> tmp/${assembly}/$tissue//$gene.TMM.matrix
done #< <(echo "$strainTEabsent")


# R

Rscript zscore.R tmp/${assembly}/$tissue//$gene.TMM.matrix $assembly $tissue $gene $position $distance

done < $wDir/INPUT/geneAnnotationsLiftoff/total_genes.lst
